        ...
        for i in range(num):
            ...
            if flag_bit in b'\x00':
                ...
                if ord(name_flags[0:1])>=128:
                    ...
                    unique = name.strip()
        return { 'group':group, 'unique':unique }
...
def smb_name(addr, port):
    if port == 135:
        ...
        if not nbns_result:
            ...
            return False
        return nbns_result['group'] + '\\' + nbns_result['unique']
...
#    # print('Build Number: %d' %  (ord(ret[start + 50:start + 51]) + 256 * ord(ret[start + 51:start + 52])))
#    msg += 'Build Number: %d' %  (ord(ret[start + 50:start + 51]) + 256 * ord(ret[start + 51:start + 52])) + '\n'
#    # 有 3 字节是空的
