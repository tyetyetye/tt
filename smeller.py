...
# sniff for echos
icmp_sniff = AsyncSniffer(iface=l_iface, prn=partial(tt_logger, 'icmp-echo'), filter="icmp[icmptype] == icmp-echo", store=0)

# sniff for tcp SYN
tcp_syn_sniff = AsyncSniffer(iface=l_iface, prn=partial(tt_logger, 'tcp-syn'), filter="tcp[tcpflags] & tcp-syn != 0", store=0)

# sniff for tcp FIN
tcp_fin_sniff = AsyncSniffer(iface=l_iface, prn=partial(tt_logger, 'tcp-fin'), filter="tcp[tcpflags] & tcp-fin != 0", store=0)
